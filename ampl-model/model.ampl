...
# Decision variable
# 1 if bin j is used, 0 otherwise
var Y{BINS} binary;          


# Obective function
# Minimize the number of bins used
minimize obective_function:
    sum{j in BINS} Y[j];
...
subject to bin_capacity{j in BINS}:
    sum{i in ITEMS} WEIGHT[i] * X[i, j] <= CAPACITY * Y[j];

# Symmetry breaking: bins are used in order
# This helps reduce search space
subject to bin_order{j in BINS: j > 1}:
    Y[j] <= Y[j - 1];
